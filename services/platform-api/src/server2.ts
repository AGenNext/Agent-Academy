...
export function buildServer(repository: PlatformRepository = new PrimitiveRepository()) {
  ...
  app.get("/api/v1/primitives/catalog", async (request) => ({ data: PrimitiveCatalog, meta: { request_id: request.id, version: "v1" } }));

  app.get("/api/v1/primitives", async (request) => {
    const QuerySchema = z.object({ kind: z.string().optional(), status: z.string().optional(), q: z.string().optional(), limit: z.coerce.number().int().positive().max(100).optional(), offset: z.coerce.number().int().nonnegative().optional() });
    const result = await repository.list(QuerySchema.parse(request.query));
  ...
  app.get("/api/v1/relationships", async (request) => {
    const query = z.object({ primitive: z.string().optional() }).parse(request.query);
  ...
  app.setErrorHandler((error, request, reply) => {
    if (error instanceof z.ZodError) return reply.status(400).send({ error: { code: "validation.failed", message: "Request validation failed", details: error.flatten() }, meta: { request_id: request.id } });
