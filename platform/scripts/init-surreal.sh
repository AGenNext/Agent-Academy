...
kubectl -n "$NAMESPACE" cp platform/surreal/schema.surql "$POD":/tmp/schema.surql
kubectl -n "$NAMESPACE" cp platform/surreal/seed.surql "$POD":/tmp/seed.surql

kubectl -n "$NAMESPACE" exec "$POD" -- surreal sql \
  --conn http://127.0.0.1:8000 \
  --user "$SURREAL_USER" \
  --pass "$SURREAL_PASS" \
  --ns "$SURREAL_NS" \
  --db "$SURREAL_DB" \
  --file /tmp/schema.surql
...
echo "SurrealDB schema and seed initialized."
